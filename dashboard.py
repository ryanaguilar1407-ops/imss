...
# Global Variables
product_data = []
selected_item = None  # Track the selected product in Treeview
...
def get_status_from_quantity(quantity):
    """Automatically determine product status based on quantity."""
    if quantity == 0:
        return "Out of Stock"
    elif quantity <= 10:
        return "Low Stock"
...
def add_product():
    ...
    if status == "Low Stock" or status == "Out of Stock":
...
def select_product(event):
    """Fill entry fields when selecting a row."""
...
def update_product():
    ...
    for item in product_data:
        if item["product"] == old_values[0] and item["quantity"] == int(old_values[1]):
...
def delete_product():
    ...
    product_data[:] = [p for p in product_data if not (p["product"] == values[0])]
    selected_item = None
...
def update_dashboard():
    """Update dashboard stats and pie chart."""
...
def update_chart():
    """Update the bar chart on the Reports page."""
